...
for k = 1:n_seg
    ...
    for t = 0:0.01:1
        ...
        for i = 0:n_order
            ...
            x_pos(idx) = x_pos(idx) + s * poly_coef_x((k-1)*(n_order+1)+i+1) * basis_p;
            y_pos(idx) = y_pos(idx) + s * poly_coef_y((k-1)*(n_order+1)+i+1) * basis_p;
        end
        idx = idx + 1;
    end
    % x_pos(idx:end) = [];
    ...
    % for i = 0:n_order-1
    %     j = (k-1)*(n_order+1)+i+1;
    %     p1 = [poly_coef_x(j), poly_coef_y(j)];
    %     p2 = [poly_coef_x(j+1), poly_coef_y(j+1)];
    %     plot_line(p1,p2);
    % end
    % j = (k-1)*(n_order+1)+1;
    % p1 = [poly_coef_x(j), poly_coef_y(j)];
    ...
    % 分段绘制Bezier曲线
    scatter(x_pos, y_pos, [], getColor(k));
    hold on;
    plot(x_pos, y_pos, 'Color', getColor(k), 'LineWidth', 1);
    idx = 1;
end
...
function color = getColor(k)
    ...
    % 检查 k 是否在有效范围内
    if k < 1 || k > size(colorMatrix, 1)
        error('k 必须在 1 到 %d 之间', size(colorMatrix, 1));
    end
