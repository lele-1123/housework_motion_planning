    ...
    for key in graph.keys():
        for child_idx, child_key in enumerate(graph[key].next_prob_1):  # or next_prob_1
            ...
            plt.arrow(graph[key].py + 0.5, graph[key].px + 0.5,
                      ...
                      color='r', head_width=0.3, head_length=0.1)  # 画箭头
            print(key, child_idx)
    plt.show()
    ...
    while bellman_error > 0.0001:
        ...
        for key in graph.keys():
            ...
            if state.is_goal:
                ...
                value_uk = []
                for child_idx in range(len(ACTION_SPACE)):  # 计算在各种输入下，该状态的G值
                    child_key_9 = state.next_prob_9[child_idx]
                    child_9 = graph[child_key_9]
...
if __name__ == '__main__':
    ...
    # solve
    dynamic_programming()
    plan1 = track_the_best_plan(0)
    visualize_the_best_plan(plan1, track_map)
